...
namespace meanran_xuexi_mods_xiaoyouhua
{
    ...
    {
        public static void 设置螺丝链接(this LogicWriterSwitch 逻辑写入开关, Interactable 逻辑写入开关控件, ILogicableReference 按钮点击返回)
        {
                ...
                case ILogicableReference内存结构.内存结构.结构类型.原始物体:
                    {
                        ...
                        {
                            case InteractableType.Button1: 设置链接物体(逻辑写入开关, 链接物); break;
                        }
                        break;
                    }
                case ILogicableReference内存结构.内存结构.结构类型.逻辑类型:
                    {
                        ...
                        switch (逻辑写入开关控件.Action)
                        { case InteractableType.Button2: 设置逻辑类型(逻辑写入开关, 逻辑类型); break; }
                        break;
                    }
            }
        }
        ...
        private static void 设置链接物体(LogicWriterSwitch 逻辑写入开关, ILogicable 选择焦点)
        {
            // TODO:联机游戏请在此处发送数据包,目前不知道应该发送什么消息
            逻辑写入开关.CurrentOutput = (Device)选择焦点;
            逻辑写入开关.LogicType = LogicType.None;
            _lastSetting.SetValue(逻辑写入开关, 0);
        }
        private static void 设置逻辑类型(LogicWriterSwitch 逻辑写入开关, LogicType 参数类型)
        {
            // TODO:联机游戏请在此处发送数据包,目前不知道应该发送什么消息
        ...
        public static 链接选择面板渲染分支选择消息.消息结构.消息类型 获取渲染分支选择消息(this LogicWriterSwitch 逻辑写入开关, Interactable 逻辑写入开关控件)
        {
            switch (逻辑写入开关控件.Action)
            {
        ...
        public static 链接选择面板渲染分支选择消息.消息结构 获取完整渲染分支选择消息(this LogicWriterSwitch 逻辑写入开关, Interactable 逻辑写入开关控件)
        {
            switch (逻辑写入开关控件.Action)
            {
                case InteractableType.Button1:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        ...
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 可链接物体表 = 逻辑写入开关.OutputNetwork1DevicesSorted.Where(d => d != (ILogicable)逻辑写入开关 && d.IsLogicWritable()) }
                    };
                case InteractableType.Button2:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        type = 链接选择面板渲染分支选择消息.消息结构.消息类型.逻辑类型渲染分支,
                        逻辑类型渲染分支消息 = new 链接选择面板渲染分支选择消息.逻辑类型渲染分支消息 { 已链接物体 = 逻辑写入开关.CurrentOutput, 只读或只写 = IOCheck.Writable }
                    };
            }
    ...
    public class LogicWriterSwitch_InteractWith_PrefixPatch
    {
        // 添加了光线命中的事件处理逻辑
        [HarmonyPrefix]
        public static bool 交互事件处理(LogicWriterSwitch __instance, ref Thing.DelayedActionInstance __result, Interactable interactable, Interaction interaction, bool doAction = true)
        {
