...
namespace meanran_xuexi_mods_xiaoyouhua
{
    ...
    {
        public static void 设置螺丝链接(this LogicSlotReader 槽位读取器, Interactable 槽位读取器控件, ILogicableReference 按钮点击返回)
        {
                ...
                case ILogicableReference内存结构.内存结构.结构类型.原始物体:
                    {
                        ...
                        { case InteractableType.Button1: 设置链接物体(槽位读取器, 链接物); break; }
                        break;
                    }
                case ILogicableReference内存结构.内存结构.结构类型.插槽编号:
                    {
                        ...
                        switch (槽位读取器控件.Action)
                        { case InteractableType.Button2: 设置插槽编号(槽位读取器, 插槽编号); break; }
                        break;
                    }
                case ILogicableReference内存结构.内存结构.结构类型.插槽类型:
                    {
                        ...
                        switch (槽位读取器控件.Action)
                        { case InteractableType.Button3: 设置插槽类型(槽位读取器, 插槽类型); break; }
                        break;
                    }
            }
        }
        private static void 设置链接物体(LogicSlotReader 槽位读取器, ILogicable 选择焦点)
        {
            // TODO:联机游戏请在此处发送数据包,目前不知道应该发送什么消息
            槽位读取器.CurrentDevice = (Device)选择焦点;
            槽位读取器.LogicSlotType = LogicSlotType.None;
            槽位读取器.Setting = 0;
            槽位读取器.SlotIndex = -1;
        }
        private static void 设置插槽编号(LogicSlotReader 槽位读取器, int 插槽编号)
        {
            // TODO:联机游戏请在此处发送数据包,目前不知道应该发送什么消息
        ...
        private static void 设置插槽类型(LogicSlotReader 槽位读取器, LogicSlotType 插槽类型)
        {
            // TODO:联机游戏请在此处发送数据包,目前不知道应该发送什么消息
        ...
        public static 链接选择面板渲染分支选择消息.消息结构.消息类型 获取渲染分支选择消息(this LogicSlotReader 槽位读取器, Interactable 槽位读取器控件)
        {
            switch (槽位读取器控件.Action)
            {
        ...
        public static 链接选择面板渲染分支选择消息.消息结构 获取完整渲染分支选择消息(this LogicSlotReader 槽位读取器, Interactable 槽位读取器控件)
        {
            switch (槽位读取器控件.Action)
            {
                case InteractableType.Button1:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        ...
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 可链接物体表 = 槽位读取器.InputNetwork1DevicesSorted.Where(d => d != (ILogicable)槽位读取器 && d.IsLogicSlotReadable()) }
                    };
                case InteractableType.Button2:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        type = 链接选择面板渲染分支选择消息.消息结构.消息类型.插槽编号渲染分支,
                        插槽编号渲染分支消息 = new 链接选择面板渲染分支选择消息.插槽编号渲染分支消息 { 已链接物体 = 槽位读取器.CurrentDevice, 只读或只写 = IOCheck.Readable }
                    };
                case InteractableType.Button3:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        type = 链接选择面板渲染分支选择消息.消息结构.消息类型.插槽类型渲染分支,
                        插槽类型渲染分支消息 = new 链接选择面板渲染分支选择消息.插槽类型渲染分支消息 { 已链接物体 = 槽位读取器.CurrentDevice, 只读或只写 = IOCheck.Readable, 插槽编号 = 槽位读取器.SlotIndex }
                    };
            }
    ...
    public class LogicSlotReader_InteractWith_PrefixPatch
    {
        // 添加了光线命中的事件处理逻辑
        [HarmonyPrefix]
        public static bool 交互事件处理(LogicSlotReader __instance, ref Thing.DelayedActionInstance __result, Interactable interactable, Interaction interaction, bool doAction = true)
        {
