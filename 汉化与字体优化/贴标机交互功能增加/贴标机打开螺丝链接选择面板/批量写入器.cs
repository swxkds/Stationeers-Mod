            ...
            switch (批量写入器控件.Action)
            {
                case InteractableType.Button1:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        ...
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 可链接物体表 = 批量写入器.OutputNetwork1DevicesSorted.Where(d => d != (ILogicable)批量写入器) }
                    };
                ...
                case InteractableType.Button3:
                    ...
                    {
                        type = 链接选择面板渲染分支选择消息.消息结构.消息类型.可链接物渲染分支,
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 可链接物体表 = 批量写入器.InputNetwork1DevicesSorted.Where(d => d != (ILogicable)批量写入器) }
                    };
            }
