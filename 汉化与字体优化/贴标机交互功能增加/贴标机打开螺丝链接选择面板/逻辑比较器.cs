            ...
            switch (逻辑比较器控件.Action)
            {
                case InteractableType.Button1:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        ...
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 可链接物体表 = 逻辑比较器.InputNetwork1DevicesSorted.Where(d => d != (ILogicable)逻辑比较器 && d is LogicUnitBase && d.IsLogicReadable()) }
                    };
                case InteractableType.Button2:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        type = 链接选择面板渲染分支选择消息.消息结构.消息类型.可链接物渲染分支,
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 可链接物体表 = 逻辑比较器.InputNetwork2DevicesSorted.Where(d => d != (ILogicable)逻辑比较器 && d is LogicUnitBase && d.IsLogicReadable()) }
                    };
