...
namespace meanran_xuexi_mods_xiaoyouhua
{
    ...
    public class TextureManager_BuildFontAtlas_Patch
    {
        ...
        public static bool Prefix(ref TextureManager __instance, ImGuiIOPtr io, in FontAtlasConfigAsset settings)
        {
            ...
            {
                if (!File.Exists(fonts[0].FontPath))
                {
                    Debug.Log("Font file not found: " + fonts[0].FontPath);
                    goto 跳过;
                }

                ImFontConfig fontConfig = default(ImFontConfig);
                ImFontConfigPtr imFontConfigPtr = new ImFontConfigPtr(ref fontConfig);
                FontConfig config = fonts[0].Config;
                config.ApplyTo(imFontConfigPtr);
                imFontConfigPtr.GlyphRanges = (IntPtr)AllocateGlyphRangeArray.Invoke(__instance, new object[] { fonts[0].Config });
                io.Fonts.AddFontFromFileTTF(fonts[0].FontPath, fonts[0].Config.SizeInPixels, imFontConfigPtr);
            }
